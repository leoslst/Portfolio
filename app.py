...
def pesquisa_estoque():
    ...
    if request.form['filtro_pesquisa'] == 'Código Lote':
        ...
        if estoques == []:
            ...
            return redirect(url_for('form_gerenciamento_estoque'))
    elif request.form['filtro_pesquisa'] == 'Loja':
    ...
    for lote in estoques:
        ...
        for produto in produtos_estoque:
            if lote.id_estoque == produto.fk_id_estoque:
                ...
                estoque[f'lote{count_lote}']['produtos'][f'produto{count_produto}']['tipo'] = tipo_produto_dao.get_tipo_produto(produto.fk_id_produto)
                estoque[f'lote{count_lote}']['produtos'][f'produto{count_produto}']['quantidade'] = produto.quantidade_produto
                estoque[f'lote{count_lote}']['produtos'][f'produto{count_produto}']['data_validade'] = produto.data_validade
            count_produto += 1
        count_lote += 1
...
@app.route('/loja/Gerenciamento', methods = ['GET','POST'])
@login_required
def form_gerenciamento_loja():
    '''
    ROTA QUE RETORNA A VIEW DE GERENCIAMENTO DE LOJAS(gerenciamento_loja.html)
    '''
    return render_template('Gerenciamento_Loja.html')
